    ...
    return (
        <nav className="sticky top-0 left-0 right-0 z-50 bg-white shadow-md">
            <div className="container mx-auto px-4 py-4 flex justify-between items-center">
                ...
                {/* Menu with Animation */}
                <div
                    className={`fixed inset-0 bg-gray-800 bg-opacity-50 z-50  ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
                        } transition-opacity duration-300`}
                    onClick={toggleMobileMenu} // Click outside to close
                >
                    <div
                        className={`w-2/3 md:w-1/2 bg-white h-full fixed  left-0 p-4 space-y-4 shadow-lg transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
                            } transition-transform duration-300`}
                        onClick={(e) => e.stopPropagation()} // Prevent click outside from closing the menu
                    >
                        <div className="flex flex-col justify-center space-y-5 w-full h-full">
                            <a
                                onClick={() => navigate('/')}
